...
const FPinConnectionResponse UAssetGraphSchema_GenericGraph::CanCreateConnection(const UEdGraphPin* A, const UEdGraphPin* B) const
{
	...
	if (EdNode_Out->GenericGraphNode->ChildrenNodes.Contains(EdNode_In->GenericGraphNode) ||
		EdNode_Out->GenericGraphNode->ParentNodes.Contains(EdNode_In->GenericGraphNode))
	{
		return FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, FText::FromString("Aready connected to this node."));
	}
	...
	if (EdNode_In->GetInputPin()->LinkedTo.Num() + EdNode_In->GetOutputPin()->LinkedTo.Num() >= 4)
	{
		return FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, FText::FromString("Maximum number of connections is 4."));
	}

	if (EdNode_Out->GetInputPin()->LinkedTo.Num() + EdNode_Out->GetOutputPin()->LinkedTo.Num() >= 4)
	{
		return FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, FText::FromString("Maximum number of connections is 4."));
	}
